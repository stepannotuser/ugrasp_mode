...
import os
import copy
class dot_generator(base_generator):
  def __init__(self):
    self.fname = "dot_generator.dot"
  ...
  def open(self,suffix):
    ...
    self.fname = self.classname + suffix
    full_filename = os.path.join(os.path.abspath(self.outdir),self.fname)
  ...
  def write_dot(self,file):
    ...
    if ( self.first_state != None ):
      self.write(file,1,f"{self.first_state} [fillcolor=lightgrey style=filled]\n")

    for idx,state in enumerate(self.db):
      _start = state.strip()
      for jdx,nextstate in enumerate(self.db[state]):
        _end   = nextstate.strip()
        self.write(file,1,f"{_start} -> {_end};\n")    
    self.write(file,0,f"}}\n")
